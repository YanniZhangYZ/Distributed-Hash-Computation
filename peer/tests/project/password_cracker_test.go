...
func Test_Password_Cracker_Simple(t *testing.T) {
	twoNode := func(t *testing.T) {
		...
		saltStr := "3c"
		err = node1.PasswordSubmitRequest(hashStr, saltStr)
		require.NoError(t, err)
		...
		saltStr = "dd"
		err = node2.PasswordSubmitRequest(hashStr, saltStr)
		require.NoError(t, err)
		time.Sleep(time.Second)
...
func Test_Password_Cracker_Multiple_Without_Leave(t *testing.T) {
	...
	for i := 0; i < len(hashStrs); i++ {
		...
		err := nodes[randomIdx].PasswordSubmitRequest(hashStrs[i], saltStrs[i])
		require.NoError(t, err)
		time.Sleep(time.Second)
